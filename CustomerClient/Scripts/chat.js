...
var chatTab = function () {
    ...
    this.showWelcomeText = function () {
        ...
        sendRequest(url, params, function (data) {
            ...
            if (data && data.length > 0) {
                ...
                $p.append(msg);
                url = String.format('/Chat/IsChatInBlack?ss={0}', customerID);
                params = { CustName: objUserData.UserName };
                sendRequest(url, params, function (r) {
                    if (r == 0) {
                        $p.append("<font color=red>正在建立连接中，请耐心等待......</font><br>");
                        _this.connectionServer();
                    }
                    else {
                        $p.append("<font color=red>感谢使用在线支持服务，请访问 http://robot.lenovo.com.cn/lenovo/think.jsp?channel=thinkshouye</font><br>");
                    }
                });
            } else {
                $p.append("<font color=red>请访问 <a href='http://support1.lenovo.com.cn'>http://support1.lenovo.com.cn</a></font><br>");
            }
        });
    }
    ...
    this.connectionServer = function () {
        ...
        sendRequest(url, params, function (data) {
            if (data != "OK.") {
                ...
                $("#pArea").append("<font color=red> 欢迎进入在线支持中心，转接中，请稍候……</font><br>");
    ...
    this.getInfo = function () {
        var url = String.format('/Chat/GetInfo?ss={0}', customerID);
        var params = { sessionID: customerID };
        lenovoTrace('GetInfostart');
        sendRequest(url, params, function (data) {
            if (data) {
                var $p = $("#pArea");
                if (data.MsgList) {
                ...
                switch (data.Status) {
                    ...
                    case -1:// 关闭聊天计时和信息获取
                        ...
                        $p.append("<div class='MsgContent'><font color='red'>您可重新进入人工咨询，重新开启会话~</font></div>");
                        $p[0].scrollTop = $p[0].scrollHeight;
