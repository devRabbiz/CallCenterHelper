...
(function ($, undefined) {
    ...
    //var emotes = { name: '默认', width: 24, height: 24, col: 7, list: { 'smile': '微笑', 'titter': '偷笑', 'laugh': '呲牙', 'tongue': '调皮', 'envy': '色', 'proud': '得意', 'shy': '害羞', 'sad': '难过', 'wronged': '委屈', 'fastcry': '快哭了', 'wail': '大哭', 'ch': '擦汗', 'baiy': '白眼', 'knock': '敲打', 'yun': '晕', 'crazy': '抓狂', 'angry': '发怒', 'ohmy': '惊讶', 'awkward': '尴尬', 'panic': '惊恐', 'cute': '可怜', 'struggle': '奋斗', 'quiet': '嘘', 'shutup': '闭嘴', 'doubt': '疑问', 'despise': '鄙视', 'sleep': '睡', 'bye': '再见' } };
    var emotes = {
        name: '默认', width: 24, height: 24, col: 7, list: {
            'smile': '微笑', 'titter': '偷笑', 'laugh': '呲牙', 'ch': '擦汗',
            'struggle': '奋斗', 'doubt': '疑问', 'bye': '再见', 'baituo': '拜托', 'baoquan': '抱拳', 'guzhang': '鼓掌', 'haixiu': '害羞', 'jingli': '敬礼', 'keai': '可爱', 'kelian': '可怜', 'kuaikule': '快哭了', 'meigui': '玫瑰', 'OK': 'OK', 'qiang': '强', 'shengli': '胜利', 'weiqu': '委屈', 'woshou': '握手', 'yongbao': '拥抱'
        }
    };
    var toolsGroups = {
        client: 'FontName,FontSize,Bold,Italic,Undercol,FontColor,|,Emote,Upload,|,HotKey',
        agent: 'FontSize,Bold,Italic,Undercol,|,Emote,QuickReply,Upload,|,HotKey,|,CaptureImage'
    };
    // 1.实例化 编辑器
    ...
    var jqeditor = function (textarea, options) {
        ...
        this.init = function () {
            ...
            $ctrl.after($('<input type="text" id="' + cursorID + '" style="position:absolute;display:none;" /><span id="' + boxID
                    ...
                    + '" class="eIframeArea" role="presentation"><div contentEditable="true" id="' + iframeID
                    + '" style="height:100%;overflow-x:hidden;overflow-y:auto;WORD-BREAK: break-all;"></div></td></tr></table></span>'));
            ...
            $iframeArea.css('height', 120);
            if (isIE & browerVer < 8) setTimeout(function () { $iframeArea.css('height', 120); }, 1);
            //激活焦点
            ...
            $bar.append(barHtml.join('')).bind('mousedown contextmenu', returnFalse)
                .click(function (event) {
                    try {
                        if ($(event.target).parent('a').attr('cmd') == 'CaptureImage') {
                            try {
                                if (top.CaptureImage) {
                                    top.CaptureImage();
                                }
            ...
            $doc = $("#" + iframeID); doc = $doc[0];
            $doc.keydown(checkHotKey).bind('mousedown click', function (ev) { $ctrl.trigger(ev.type); });
            $doc.focus(function () {
                top.isEditorFocus = true;
            }).blur(function () {
                top.isEditorFocus = false;
            });
            //自动清理粘贴内容
            $(doc.ownerDocument.documentElement).bind(isIE ? 'beforepaste' : 'paste', function () {
        ...
        this._exec = function (cmd, param, noFocus) {
            ...
            var state;
            // 执行命令标识符
            if (param !== undefined) state = $(document)[0].execCommand(cmd, false, param);
            else state = $(document)[0].execCommand(cmd, false, null);
            return state;
        }
        ...
        this.showEmote = function () {
            ...
            $emoteDialog = $('<style> .eEmote div a{width:' + emotes.width + 'px;height:' + emotes.height + 'px;}</style><div class="eDialog"><div class="eBox"><h3>请选择表情</h3><div class="eEmote"><div>' + imgList.join('') + '</div></div></div></div>')
                .click(function (ev) {
                    ev = ev.target;
        ...
        // 常用语
        this.showQuickReply = function () {
            var jQuickReply = $('<div class="eBox"><h3>请选择内容模板（将替换当前内容）</h3><div class="eList"></div></div>');
            var jList = $('.eList', jQuickReply), items = []
            if (words) {
                jList.html("");
                $.each(words, function (i) {
                    items.push('<div tabindex="0" title="' + words[i] + '">' + words[i] + '</div>');
                });
                jList.append(items.join(''));
                $("div", jList).click(function (ev) {
                    doc.innerHTML = '<font size=\"' + lastFontSize + '\" face=\"' + lastFontName + '\">' + $(this).html() + '</font>';
                    _this.hidePanel();
                });
            }
            _this.showDialog(jQuickReply);
        }

        // 文件上传
        this.showUpload = function () {
            var idIO = 'jUploadFrame' + new Date().getTime(), _this = this;
            var jIO = $('<iframe name="' + idIO + '" src="/domain.aspx" class="eHideArea" />').appendTo('body');
            ...
            jUpBtn.click(function () {
                ...
                var upload = new _this.html4Upload(jFile[0], "/Chat/FileUpload2", uploadCallback);
                upload.start();
            });
        ...
        this.html4Upload = function (inputfile, toUrl, callback) {
            ...
            var jIO = $('<iframe name="' + idIO + '" src="/domain.aspx" class="eHideArea" />').appendTo('body');
            var jForm = $('<form action="' + toUrl + '" target="' + idIO + '" method="post" enctype="multipart/form-data" class="xheHideArea" style="display:none"></form>').appendTo('body');
        ...
        this.showHotKey = function () {
            var $hotKey = $('<div class="eBox"><h3>请选择发送快捷键</h3><div class="eList">'
        ...
        //截图
        this.showCaptureImage = function () {
            var jCaptureImage = $('<div class="eBox"><h3>屏幕截图</h3><div class="eList">注：截图过程中按ESC退出<br/><input type="button" value="开始截图" class="eBtn" tabindex="-1" /></div></div>');
            var jUpBtn = $('.eBtn', jCaptureImage);
            jUpBtn.click(function () {
                try {
                    if (top.CaptureImage) {
                        top.CaptureImage();
            ...
            _this.showDialog(jCaptureImage);
        }
        ...
        this.getRng = function (bNew) {
            ...
            try {
                ...
                if (!rng) rng = $(document)[0].createTextRange ? $(document)[0].createTextRange() : $(document)[0].createRange();
            } catch (ex) { }
            return rng;
        }
        this.getSel = function () { return $(document)[0].selection ? $(document)[0].selection : window.getSelection(); }


        this.getContentAndClear = function () {
            var text = '';
            $text = $("<div></div>");
            $text.append(doc.innerHTML);
            if ($text.length > 0) {
            ...
            if ($($text[0]).text().length > 350) {
                ...
                doc.innerHTML = '<font size=\"' + lastFontSize + '\" face=\"' + lastFontName + '\"></font>';
                _this.saveBookmark();
                _this.loadBookmark();
            }
            return text;
